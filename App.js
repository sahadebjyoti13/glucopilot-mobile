...
function TabIcon({ name, focused }) {
  const icons = { Home: '⬤', Trends: '📈', Exercise: '🏃', Profile: '👤', Settings: '⚙️' };
  return (
...
export default function App() {
  ...
  const requireBiometric = async () => {
    ...
    if (result.success) setAuthState('authenticated');
    else if (result.error === 'user_cancel') {
      await TokenStore.clear();
      setAuthState('logged_out');
    } else {
      Alert.alert('Authentication Required', 'Fingerprint not recognised. Try again.', [
  ...
  if (authState === 'checking') return <View style={s.splash}><Text style={s.splashTitle}>GlucoPilot</Text><Text style={s.splashSub}>Loading…</Text></View>;
  if (authState === 'logged_out') return <SafeAreaProvider><LoginScreen onAuth={async () => { await requireBiometric(); }} /></SafeAreaProvider>;
  if (authState === 'needs_biometric') return <View style={s.splash}><Text style={{ fontSize: 60, marginBottom: 16 }}>👆</Text><Text style={s.splashTitle}>GlucoPilot</Text><Text style={s.splashSub}>Verifying identity…</Text></View>;

  return (
    <SafeAreaProvider>
      <StatusBar barStyle="light-content" backgroundColor="#0a0e1a" />
      <NavigationContainer theme={{ colors: { background: '#0a0e1a', card: '#111827', text: '#f0f4ff', border: 'rgba(255,255,255,0.07)', primary: '#00d4aa', notification: '#ff4757' }, dark: true }}>
        <Tab.Navigator
          screenOptions={({ route }) => ({
            tabBarIcon: ({ focused }) => <TabIcon name={route.name} focused={focused} />,
            tabBarStyle: { backgroundColor: 'rgba(10,14,26,0.95)', borderTopColor: 'rgba(255,255,255,0.07)', borderTopWidth: 0.5, paddingBottom: 20, height: 70 },
...
const s = StyleSheet.create({
  splash: { flex: 1, backgroundColor: '#0a0e1a', alignItems: 'center', justifyContent: 'center' },
